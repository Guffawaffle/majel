...
- Added per-target episodic continuity snapshots in `list_targets` by carrying forward recent correction deltas plus reminder feedback context (`continuity`, `recentReminderFeedback`) for active-goal follow-up turns. (`src/server/services/fleet-tools/read-tools.ts`, `test/fleet-tools.test.ts`)
